...
public class firstYearNotices extends AppCompatActivity implements AdapterView.OnItemClickListener {
        ...
        if(requestCode == GALLERY_INTENT && resultCode == RESULT_OK){
            ...
            if(data.getClipData()!=null){
                ...
                for(int i=0;i<totalItemsSelected;i++){
                    ...
                    StorageReference filetopath = multipleImageStorage.child("Notices").child("firstYear").child(filePathValue).child(s);
                    filetopath.putFile(uri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        @Override
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            Toast.makeText(firstYearNotices.this, "done", Toast.LENGTH_SHORT).show();
                ...
                uploadProgress.show();
                final String filePathValue = FilePathName.getText().toString();
                StorageReference filepath = imageStorage.child("Notices").child("firstYear").child(filePathValue).child("0");
                filepath.putFile(uri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    @Override
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        Toast.makeText(firstYearNotices.this, "Upload Done", Toast.LENGTH_SHORT).show();
                        uploadProgress.dismiss();
                        Date todaysDate = new Date();
                        DateFormat df2 = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
                        String date = df2.format(todaysDate);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("First Year notices");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        Toast.makeText(firstYearNotices.this, "Click on a specific notification to open it for the detailed information about the notification", Toast.LENGTH_SHORT).show();
        ...
        imageButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(myedittext.getText().toString().trim().length()>0) {
                    ...
                    Toast.makeText(firstYearNotices.this, "Enter the notice before adding image", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.favorite:
                Toast.makeText(firstYearNotices.this, "Added to your favorite", Toast.LENGTH_SHORT).show();
                break;
            case R.id.about:
